...
// @ts-expect-err2or - This package lacks type definitions.
// import importPlugin from 'eslint-plugin-import'
// @ts-expect-er2ror - This package lacks type definitions.
 ...
 */
export function getCommonRules(): Rules {
  const rules: Rules = {}
  ...
  // rules['no-async-promise-executor'] = 'off'
  Reflect.deleteProperty(rules, 'no-async-promise-executor')
  ...
  Reflect.deleteProperty(rules, 'arrow-body-style')
  ...
  rules['@stylistic/array-bracket-newline'] = ['error', 'consistent']

  // Disallow spaces inside array brackets.
  rules['@stylistic/array-bracket-spacing'] = ['error', 'never']

  // Require parens around arrow function arguments only when required.
  rules['@stylistic/arrow-parens'] = ['error', 'as-needed', {
  ...
  rules['@stylistic/arrow-spacing'] = ['error', {
    before: true,
    after: true
  }]
  ...
  // same line.
  rules['@stylistic/block-spacing'] = ['error', 'always']

  // Enforce the usage of 'one-true-brace-style' for braces.
  rules['@stylistic/brace-style'] = ['error', '1tbs']
  ...
  rules['@stylistic/comma-spacing'] = ['error', {
    before: false,
    after: true
  }]
  ...
  // declarations.
  rules['@stylistic/comma-style'] = ['error', 'last']

  // Disallow spaces inside of computed properties in object literals.
  rules['@stylistic/computed-property-spacing'] = ['error', 'never', {
    enforceForClassMembers: true
  }]

  // Require a newline at the end of files.
  rules['@stylistic/eol-last'] = ['error', 'never']

  // Disallow spaces between the function name and the opening parenthesis that
  // calls it.
  rules['@stylistic/func-call-spacing'] = ['error', 'never']
  ...
  rules['@stylistic/jsx-quotes'] = ['error', 'prefer-double']

  // Require a space before and after keywords like `for`, `if`, etc.
  rules['@stylistic/keyword-spacing'] = ['error', {
    before: true,
    after: true
  }]

  // Warn about long lines.
  rules['@stylistic/max-len'] = ['warn', {
    tabWidth: 2,
  ...
  Reflect.deleteProperty(rules, '@stylistic/max-statements-per-line')

  // Require no semi-colons after members in interface declarations.
  rules['@stylistic/member-delimiter-style'] = ['error', {
    multiline: {
      delimiter: 'none',
      requireLast: false
    },
  ...
  rules['@stylistic/no-confusing-arrow'] = ['error', {
  ...
  rules['@stylistic/no-extra-parens'] = ['error', 'all', {
    ignoreJSX: 'all',
    enforceForArrowConditionals: false
  }]

  // Disallow unnecessary semicolons.
  rules['@stylistic/no-extra-semi'] = 'error'

  // Allow up to 1 consecutive empty line.
  rules['@stylistic/no-multiple-empty-lines'] = ['error', { max: 1 }]
  ...
  rules['@stylistic/rest-spread-spacing'] = 'error'

  // Do not use semi-colons.
  rules['@stylistic/semi'] = ['error', 'never']

  // Enforce consistent spacing before function parenthesis.
  rules['@stylistic/space-before-function-paren'] = ['error', {
    named: 'never',
    anonymous: 'never',
    asyncArrow: 'always'
  }]
  ...
  Reflect.deleteProperty(rules, '@stylistic/multiline-ternary')
  ...
  // rules['import/no-dynamic-require'] = 'off'
  Reflect.deleteProperty(rules, 'import/no-dynamic-require')
  ...
  rules['import/no-useless-path-segments'] = 'error'
  ...
  Reflect.deleteProperty(rules, 'import/no-named-as-default-member')
  Reflect.deleteProperty(rules, 'import/no-named-as-default')
  ...
  Reflect.deleteProperty(rules, 'import/no-extraneous-dependencies')
  ...
  // (libraries).
  Reflect.deleteProperty(rules, 'import/no-unused-modules')

  // These rules are far too ambitious to exist right now.
  Reflect.deleteProperty(rules, 'import/no-commonjs')
  Reflect.deleteProperty(rules, 'import/no-amd')
  Reflect.deleteProperty(rules, 'import/no-nodejs-modules')
  Reflect.deleteProperty(rules, 'import/no-import-module-exports')

  // Disable; conflicts with 'import/order' rule.
  Reflect.deleteProperty(rules, 'import/first')

  // This rule enforces that all exports are declared at the bottom of the file.
  // This rule will report any export declarations that comes before any
  // non-export statements.
  Reflect.deleteProperty(rules, 'import/exports-last')
  ...
  Reflect.deleteProperty(rules, 'import/no-whitespace')

  // Do not require the use of file extensions within the import path.
  Reflect.deleteProperty(rules, 'import/extensions')
  ...
  Reflect.deleteProperty(rules, 'import/no-unassigned-import')
  ...
  Reflect.deleteProperty(rules, 'import/no-named-default')

  // Allow default exports.
  Reflect.deleteProperty(rules, 'import/no-default-export')

  // Allow named exports.
  Reflect.deleteProperty(rules, 'import/no-named-export')

  // Allow anonymous default exports (again).
  Reflect.deleteProperty(rules, 'import/no-anonymous-default-export')

  // Do not require exports to be grouped.
  Reflect.deleteProperty(rules, 'import/group-exports')
  ...
  rules['prefer-arrow/prefer-arrow-functions'] = ['error', {
  ...
  Reflect.deleteProperty(rules, 'unicorn/consistent-function-scoping')
  ...
  Reflect.deleteProperty(rules, 'unicorn/no-array-callback-reference')

  // Allow the use of Array#forEach().
  Reflect.deleteProperty(rules, 'unicorn/no-array-for-each')

  // [Dec 2021] This rule is throwing false positives with utility libraries
  // like Ramda, so it has been temporarily disabled.
  Reflect.deleteProperty(rules, 'unicorn/no-array-method-this-argument')

  // Allow consecutive calls to .push() on the same array.
  Reflect.deleteProperty(rules, 'unicorn/no-array-push-push')

  // Allow use of Array.prototype.reduce.
  Reflect.deleteProperty(rules, 'unicorn/no-array-reduce')

  // Allow member access of parenthesized await expressions.
  Reflect.deleteProperty(rules, 'unicorn/no-await-expression-member')

  // Allow nested ternary expressions.
  Reflect.deleteProperty(rules, 'unicorn/no-nested-ternary')
  ...
  Reflect.deleteProperty(rules, 'unicorn/prefer-module')

  // Top-level await is only available in ESM/ESNext contexts.
  Reflect.deleteProperty(rules, 'unicorn/prefer-top-level-await')

  // Don't enforce 'more descriptive' variable names.
  Reflect.deleteProperty(rules, 'unicorn/prevent-abbreviations')

  // Allow
  Reflect.deleteProperty(rules, 'unicorn/prefer-object-from-entries')

  // Do not require that "TODO" comments have an expiration date.
  Reflect.deleteProperty(rules, 'unicorn/expiring-todo-comments')

  // Only require braces in a switch's "case" statements when they are required.
  rules['unicorn/switch-case-braces'] = ['error', 'avoid']

  // Do not enforce rules around await-ing non-Promise values.
  Reflect.deleteProperty(rules, 'unicorn/no-unnecessary-await')

  // Allow negated conditions.
  Reflect.deleteProperty(rules, 'unicorn/no-negated-condition')

  // Do not enforce import style.
  Reflect.deleteProperty(rules, 'unicorn/import-style')

  // Allow anonymous default exports.
  Reflect.deleteProperty(rules, 'unicorn/no-anonymous-default-export')

  // Allow abbreviated variable/function names.
  // Reflect.deleteProperty(rules, 'unicorn/prevent-abbreviations')
