...
// consumers via their peerDependencies so the consumer does not have to manage
// their versions.
config.rules['import/no-extraneous-dependencies'] = 'off';

// Allow modules to have no exports (tests, CLI entry points) and allow modules
// to export values that are not imported in the local project (libraries).
...
// [DISABLED]
// This rule is disabled because it parses every file in scope according to the
// parserOptions.sourceType setting. Many projects still need to have mixed
// module syntax and CJS syntax, especially for configuration files like this
// one which configure tools such as ESLint, which as of yet have refused to
// implement support for configuration files in ESM syntax.
...
// Allow namespace imports. This is the most convenient way to use packages like
// Ramda without renaming named imports to avoid conflicts.
...
// Allow named values to be the default export of a module. This can often help
// improve the readability of stack traces for debugging.
...
// Don't enforce usage of the 'node:' protocol for importing built-ins; we still
