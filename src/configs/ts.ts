...
export const ts = defineConfig([{
  ...
  rules: {
    ...
    // strictly requires quotes, otherwise disallow quotes around property
    // names.
    ...
    // module. TypeScript's type inference is good enough that this shouldn't be
    // necessary.
    ...
    '@typescript-eslint/naming-convention': ['warn', {
      ...
      // often need to use objects whose shape is defined by a third-party API
      // or schema that we have no control over.
      selector: 'property',
    ...
    '@typescript-eslint/no-namespace': ['error', {
      ...
      // custom "modules" for non-code file types (ie: images) to avoid
      // TypeScript errors when they are imported.
    ...
    // modules/APIs in the JavaScript ecosystem, and the need for `any` is still
    // quite common.
    ...
    // have the 'noImplicitAny' TypeScript option enabled and will therefore
    // have to explicitly declare the return type as `any`.
    ...
    // have unintended results at times, but also allows developers to implement
    // their own toString methods on objects that will serialize them in a sane
    // way.
